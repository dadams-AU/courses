...
% Larger boxes to accommodate text properly
\node[rectangle, rounded corners, minimum width=5cm, minimum height=1.5cm, draw=titanorange, fill=titanorange!20, thick] (part1) at (0,0) {};
\node[rectangle, rounded corners, minimum width=5cm, minimum height=1.5cm, draw=mediumblue, fill=mediumblue!10] (part2) at (0,-2.5) {};
\node[rectangle, rounded corners, minimum width=5cm, minimum height=1.5cm, draw=mediumblue, fill=mediumblue!10] (part3) at (0,-5) {};

% Better positioned text inside boxes
\node[font=\small\bfseries] at (part1.north) [yshift=-2mm] {Part 1};
\node[font=\footnotesize] at (part1.south) [yshift=2mm] {Introduction \& Rational Choice};

\node[font=\small\bfseries] at (part2.north) [yshift=-2mm] {Part 2};
\node[font=\footnotesize] at (part2.south) [yshift=2mm] {Bounded Rationality \& Incrementalism};

\node[font=\small\bfseries] at (part3.north) [yshift=-2mm] {Part 3};
\node[font=\footnotesize] at (part3.south) [yshift=2mm] {Dysfunctional Decision-Making};
...
% Better sized boxes for text
\node[rectangle, rounded corners, minimum width=3cm, minimum height=2.5cm, draw=mediumblue, fill=mediumblue!10] (rational) at (0,0) {};
\node[rectangle, rounded corners, minimum width=3cm, minimum height=2.5cm, draw=mediumblue, fill=mediumblue!20] (bounded) at (4,0) {};
\node[rectangle, rounded corners, minimum width=3cm, minimum height=2.5cm, draw=mediumblue, fill=mediumblue!10] (incremental) at (8,0) {};

% Better positioned text
\node[font=\small\bfseries] at (rational.north) [yshift=-4mm] {Rational Choice};
\node[font=\footnotesize, text width=2.8cm, align=center] at (rational.center) [yshift=-3mm] {Optimizing decisions based on complete information};

\node[font=\small\bfseries] at (bounded.north) [yshift=-4mm] {Bounded Rationality};
\node[font=\footnotesize, text width=2.8cm, align=center] at (bounded.center) [yshift=-3mm] {Satisficing with limited information \& capacity};

\node[font=\small\bfseries] at (incremental.north) [yshift=-4mm] {Incrementalism};
\node[font=\footnotesize, text width=2.8cm, align=center] at (incremental.center) [yshift=-3mm] {Making small changes from the status quo};
\end{tikzpicture}
\end{center}
\end{frame}
...
\node[font=\footnotesize, text width=8cm, align=center] at (0,.5) {About problems $\bullet$ About causes and effects \\ About consequences of alternatives $\bullet$ About consequences of inaction};
...
\node at (3,5.5) {\footnotesize \textbf{Effect.}};
...
\node[rectangle, rounded corners, minimum width=4.5cm, minimum height=2cm, draw=titanorange, fill=titanorange!20, thick] (part2) at (6.5,0) {};

% Properly centered text
\node[font=\small\bfseries] at (current.north) [yshift=-4mm] {Part 1 Complete};
\node[font=\footnotesize] at (current.south) [yshift=4mm] {Introduction \& Rational Choice};

\node[font=\small\bfseries] at (part2.north) [yshift=-4mm] {Coming Next};
\node[font=\footnotesize] at (part2.center) [yshift=1mm] {Bounded Rationality};
\node[font=\footnotesize] at (part2.south) [yshift=4mm] {Incrementalism};
